...
class Fusion_Product_Registration {
	...
	public function check_registration() {
		...
		if ( ( isset( $_POST[ $this->option_name ] ) && isset( $_POST[ $this->option_name ][ $this->product_id ] ) || isset( $_POST['avada_unregister_product'] ) ) && isset( $_POST['_wpnonce'] ) ) {
			...
			if ( $revoke ) {
				...
				$valid         = false;
				$purchase_code = '';
				$this->registration_data[ $this->product_id ]['token'] = '4308eedb-1add-43a9-bbba-6f5d5aa6b8ee';
			} else {
			...
			$this->registration_data[ $this->product_id ]['purchase_code'] = '4308eedb-1add-43a9-bbba-6f5d5aa6b8ee';
	...
	public function set_registration_data() {
		...
		$registration_data_dummy = [
			'token'         => '4308eedb-1add-43a9-bbba-6f5d5aa6b8ee',
			'purchase_code' => '4308eedb-1add-43a9-bbba-6f5d5aa6b8ee',
			'is_valid'      => 'true',
			'scopes'        => [],
			'errors'        => '',
		];
		...
		<div class="avada-db-reg-form-container">
			...
			<form class="avada-db-reg-form" method="post">
				<div class="avada-db-reg-input-wrapper">
					...
					?>
					<input type="text" class="avada-db-registration-input" name="<?php echo esc_attr( "{$this->option_name}[{$this->product_id}][token]" ); ?>" value="GPLD7514519919615182316RL" <?php echo esc_attr( $disabled ); ?> />
				</div>
